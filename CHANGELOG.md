...
**验证**：e2e 268 → 273，五条断言（不再判定 API 不可用 / 答案数量对齐 / 形状合法 /
不碰答案缓存 / 关掉后恢复 AI 路径）。
反向验证：去掉 3 处守卫 → 2 条立刻失败（`{"reason":"api-key-missing"}` 与 `{"reads":2}`）。
