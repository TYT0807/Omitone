...
**卡在哪一步了？** → [在线说明书](https://tyt0807.github.io/Omitone/docs/manual.html) 里第 2 节有每一步的界面截图，
...
| **`使用说明.pdf`** | **给使用者的图文说明书 · PDF 版**（14 页，含界面示意图）。放在仓库根目录方便一眼找到；适合离线看、转发给同学。**在线看请直接用下面那份 HTML**（PDF 在 GitHub 上会"赌运气"渲染，见下方说明） |
| [`docs/manual.html`](docs/manual.html) | **给使用者的图文说明书 · 在线版**（README 顶部「在线看图解说明书」指向 GitHub Pages 上的它），同时也是上面那份 PDF 的**排版源文件**（内联 SVG、A4 打印 CSS，零外部依赖）。改内容改这个，再跑 `npm run manual` 重新出 PDF |
