...
| **乱选（不接 AI）** | 不想配 API Key 也能用：本地随机作答，不保证正确率，只求把卷交出去。随机的答案**不会写进答案记忆**，将来换成 AI 答题不受影响 |
...
「自动化验证」列指 `npm run e2e`（真实 Edge + 26 个场景，273 项断言）覆盖到哪一步。
...
npm test         # 上面三个
npm run e2e      # 真实 Edge 功能交叉检验（26 个场景 / 273 项）
