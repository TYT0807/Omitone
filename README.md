...
![版本](https://img.shields.io/badge/版本-1.2.5-blue)
...
| **拖到结尾** | 可拖动的视频/音频直接 seek 到片尾，每个只试一次 |
...
「自动化验证」列指 `npm run e2e`（真实 Edge + 29 个场景，311 项断言）覆盖到哪一步。
...
| **拖到结尾** | 可拖动的视频/音频直接 seek 到 `duration - 3s`，每个只试一次 | 时机判断、开关生效 | ✅ |
...
除了判定顺序，还有两件事属于"改 `_runTick` 前必须懂"：

- **重入锁 `_tickRunning`**：保证同一时刻只有一个 tick 在跑。看门狗
  （`_startTickLoop` 里"挂起超 150 秒就强制放锁"）是**对的** —— 历史 bug 就靠它兜住。
  但要注意：被强制放锁的那次 tick **并没有真的结束**，它只是还在挂起。
- **代号 `_tickEpoch`**：正因如此，`finally` 里复位锁必须**先确认代号还是自己的**。
  否则旧 tick 过一会儿醒来时会把**新 tick 的锁**一起清掉 → 下一轮又起一个 tick →
  **两个 tick 并行推进**（可能重复提交、重复跳章）。
  ⚠️ **改 `_runTick` 的 `finally` 时别把这个代号判断删掉。**
  正常路径下代号必然相等，所以这段判断不影响任何正常行为。
...
npm run check    # 工程自检：语法 / manifest / 版本一致性 / 编码损坏 / 幽灵调用 / 死方法 / 死代码 / 唯一真源 / 产物与片段一致 / 片段头部一致 / await 可超时
npm run bench    # 提示词 token 基准（三代对比 + 信息完整性自检）
npm run itest    # 集成测试：真实 content.js 的答题往返（77 项）
npm test         # 上面三个
npm run e2e      # 真实 Edge 功能交叉检验（29 个场景 / 311 项）
...
| 层 | 手段 | 能测出什么 | 成本 |
| --- | --- | --- | --- |
| 静态自检 | `tools/check.js` | 语法、版本一致性、**幽灵调用**、死方法、编码损坏、唯一真源、**产物与片段一致**、**片段头部一致**、**await 可超时** | 秒级 |
