...
> **1.2.11 修复**：任务点**匹配得上、却跑不起来**的一种页面写法（身份号挂在容器元素上时，反查不到它对应的框架）。
...
> 偶尔一题拿不到答案、白花一次请求。
> **1.2.5 新增**：**音频**也能拖到结尾（与视频同一套逻辑）。
> **1.2.4 修复**：讨论任务**新开的窗口**里插件不自动跑。
> [完整变更记录 →](CHANGELOG.md)
...
「自动化验证」列指 `npm run e2e`（真实 Edge + 31 个场景，327 项断言）覆盖到哪一步。
