...
> 少数开关目前**只存在于配置里，弹窗界面还没做**：防拖拽 90% 提前结束、媒体 / 文档 /
> 超链接任务点、被换页自动恢复。它们默认都是开的，想关要改 `chrome.storage.local`
> 里的 `config`；后续版本会把开关补进弹窗。
...
「自动化验证」列指 `npm run e2e`（真实 Edge + 19 个场景，187 项断言）覆盖到哪一步。
