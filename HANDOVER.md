...
### 0.2 交接时的工作区状态（快照：2026-09-18 傍晚）

> 这一段是**时间点快照**，会过期。先跑 `git log --oneline -3` 和 `git status --short`
> 自己对一下；如果工作区干净、远端 main 已经跟上，这整节可以跳过。
>
> **本次交接后已推进到**：`1.1.6` 已发布并核验（工作区干净、`main` 与 `v1.1.6` 一致）。

- **1.1.2 / 1.1.3 / 1.1.4 / 1.1.5 均已发布并核验过**。
  下载链接按**附件名**取「最新一版 Release」，所以点下载拿到的是 1.1.5。
- **1.1.6 已发布并核验过**（`main` = tag `v1.1.6`，两个附件 `state=uploaded`）。内容四块：
...
- 待提交的改动大致是这些：
  ...
  ```
  修改: page.js                        （_frameHasTaskPoint / _locateDocumentTask 任务点闸门 /
                                        _searchChaoxingJobOcs 补 pagedDoc /
                                        _handleDocumentTask 完成后推进 / 视觉模块五件）
        content.js                     （logTokenUsage + 验证码三分支记账 +
                                        handleVisionRequestDirect + llm_vision 分发）
        popup/popup.html popup/popup.js（字体放大 + 视觉开关/模型/预算三控件）
        manifest.json                  （版本号 name / default_title）
        README.md                      （视觉章节 + 功能表 + 页数 13→14）
        HANDOVER.md  CHANGELOG.md      （1.1.6 记录）
        使用说明.pdf                    （重新生成，版本盖成 1.1.6）
        package.json                   （版本号）
  ```
- 发新版（**先读 §0.5 的令牌规矩**）—— 下面是**已完成**的本轮实际操作，留作下次照抄：

  ```bash
  npm run release -- check-msg .workbuddy/commit-msg.txt   # 先校验提交信息形状（不联网）
  npm run release -- push --message-file <提交信息文件> <上面那些文件...>
  npm run release -- release v1.1.6 --notes-file <Release说明文件>
  npm run release -- verify v1.1.6        # 必须跑，确认附件 state 与下载直链
  ```

  注意 `push` 子命令**没有** `git add -A` 那种"全都提交"的用法 —— 文件必须一个个列出来。
  这是刻意的：本项目误提交过自检产物（见 §7.5）。

- 交接前的整目录备份在 **`D:\Omite-backup-20260918-1905`**。
...
**还没做（下一步）：把说明书重写成"担架级"教程。**
读者假设是**完全没上过网的人**：每一步都要写清
「看到什么 → 点哪里 → 出现什么才算对 → 不对怎么办」。
入口问题（上面这些）已解决，重写是**内容层面**的事，可以独立推进。
...
- 改了手册内容**页数会变**，README 文档地图里写了页数（现在是 14 页），记得同步
