...
#### ⏳ 最新状态（2026-09-23）—— 1.2.6 已就绪，**但还没推送**

**当前版本 1.2.6（本地）**。远端 `main` 仍停在 `a922b0c`（远端最新 tag 是 **v1.2.3**）——
因为**发版令牌已被 revoke**（`~/.omitone-release.ghtoken` 不存在），推不了。

**本地领先远端 8 个提交**，其中 3 个是"改了但没发出去"的版本：

| 版本 | 内容 | 状态 |
| --- | --- | --- |
| 1.2.4 | 讨论新开的窗口里插件不自动跑（`run()` 落续跑标记） | 未推送 |
| 1.2.5 | 音频也能拖到结尾（与视频一视同仁） | 未推送 |
| 1.2.6 | 修诊断里写错的选择器大小写 + 三条守卫加固（见下） | 未推送 |

**拿到新令牌后**：一次 `git push`（走 AGENTS §7.3 那条"令牌放进 URL"的路，保留完整历史）
+ 发一个 **v1.2.6** 即可覆盖这三个版本。
...
| | |
| --- | --- |
| 本地 / 远端 | **本地领先 8 个提交**；远端 `main` = `a922b0c`（= v1.2.3） |
| 工作区 | 干净 |
| 线上 Release | **仍是 v1.2.3** —— 1.2.4 / 1.2.5 / 1.2.6 **都还没发** |
| 发布产物 | 已就绪并校验：`dist/omitone-1.2.6.zip`（369.0 KB）、`使用说明.pdf`（1.39 MB）；包内 `page.js` / `background.js` / `content.js` 与仓库根目录**逐字节一致** |
