...
- **最新一次提交是 `1742b9f 修作业页回填与提交后判定`**（远端 `main` 已跟上，9 个文件逐个比对字节一致）。
  ...
  端到端现在是 **26 个场景 / 262 项**，新增「作业页选项写入隐藏答案域」（8 项）
  与「作业页提交后认出已完成」（9 项）。
- **测试基线：自检 13 项 · 集成 77 项 · 真实 Edge e2e 262 项（26 个场景）· 提示词基准通过**。
...
- **`dist/` 已按新代码重新打包**：`dist/omitone-1.1.6.zip` 353.4 KB（18 个文件），
  ...
  ⚠️ **线上 Release 附件（zip 356.7 KB / PDF 1386.8 KB）是本轮修复**之前**的旧包**，
  如果用户要让"点下载拿到的就是修好的版本"，需要**删掉旧附件、用 `dist/` 里这份重传同名**
  （参照 `892e6d0` 那次只换 PDF 的做法，不用重发整版），否则用户拿到的还是旧的。
  这一步**还没做** —— 要不要发，交给下一个 AI 判断（详见 §9 第一行）。
...
- ⚠️ **`0f345a2` 修的是上面 `8ce8dae` 没修透的那一半**，值得单列一条：
  ...
  **从来没读过这个字段** —— 拿回 `{success:false, permanentError:true}` 之后
  ...
  测试基线：自检 13 项 · 集成 **77** 项 · 真实 Edge e2e **262 项**（26 个场景）。
...
| 项 | 说明 | 建议 |
| --- | --- | --- |
| **线上 Release 附件是修复前的旧包** | 本轮（`1742b9f`）修了作业页回填与提交后判定，`dist/omitone-1.1.6.zip` 已按新代码重打（353.4 KB），但**没重发** —— 用户点下载拿到的 zip/PDF 仍是旧的 | 二选一：① 用户确认要在真实平台见效后，删旧附件、重传**同名**新附件（不重发整版，参照 `892e6d0` 那次只换 PDF 的做法）；② 攒到下个版本一起发。流程照 §0.2 末尾的命令清单 |
| **本轮修复还没在真实平台验证过** | 「作业页回填隐藏域」「提交后认判分结果页」两个修复只过了 mock e2e（262 项全绿 + 反向验证），真实学习通作业/考试页**用户尚未回归实测** | 等用户跑一次真实作业页。若仍有异常，先按 README §7 调试手册新增的两行（`#answer{qid}` 空串 / 停在 `waiting quiz submit result`）定位，别直接改代码 |
