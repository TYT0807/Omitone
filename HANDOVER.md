...
- **最新一次提交是 `1742b9f 修作业页回填与提交后判定`**（远端 `main` 已跟上，9 个文件逐个比对字节一致）。
  ...
  端到端现在是 **26 个场景 / 266 项**，新增「作业页选项写入隐藏答案域」（8 项）
  与「作业页提交后认出已完成」（9 项）。
- **测试基线：自检 13 项 · 集成 77 项 · 真实 Edge e2e 266 项（26 个场景）· 提示词基准通过**。
...
- ⚠️ **`0f345a2` 修的是上面 `8ce8dae` 没修透的那一半**，值得单列一条：
  ...
  **从来没读过这个字段** —— 拿回 `{success:false, permanentError:true}` 之后
  ...
  测试基线：自检 13 项 · 集成 **77** 项 · 真实 Edge e2e **266 项**（26 个场景）。
...
| 项 | 说明 | 建议 |
| --- | --- | --- |
| **本轮修复还没在真实平台验证过** | 「作业页回填隐藏域」「提交后认判分结果页」两个修复只过了 mock e2e（266 项全绿 + 反向验证），真实学习通作业/考试页**用户尚未回归实测** | 等用户跑一次真实作业页。若仍有异常，先按 README §7 调试手册新增的两行（`#answer{qid}` 空串 / 停在 `waiting quiz submit result`）定位，别直接改代码 |
