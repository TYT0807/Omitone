...
- **1.1.3 已发布并核验过**（tag / Release / 两个附件 state=uploaded / 下载直链可用）。
  下载链接按附件名取「最新一版 Release」，所以现在点下载拿到的就是 1.1.3。
...
| 答题链路 / 抠题 / 媒体 / 任务点调度 / `content.js` | `npm run e2e` | **193 / 193**（约 1~2 分钟，会起一个独立 Edge 临时 profile，不碰你正在用的浏览器） |
...
| 测试基线 | 自检 13 项 · 集成 52 项 · 真实 Edge 端到端 **193 项** · 提示词基准 1 份报告，**全绿** |
...
npm run e2e                   # 约 1~2 分钟：起一个独立 Edge 临时 profile，193 项交叉检验
```
