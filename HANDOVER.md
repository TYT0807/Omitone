...
> 下面这一整段是 **1.1.6 发布时**的快照：里面的版本号、提交号、附件 sha256 都已过期，
> 只用来理解"上一版是怎么发出去的"。**当前状态看上面那段。**

**本地与远端已完全同步，线上附件也已换成含「乱选」的新包。**
（快照写于 2026-09-21 傍晚；之后又加了 **7 个提交**（`dcd264d` 起）—— 其中 `dcc44fe`
把 `page.js` 拆成 12 个按域片段、`2bb6332` 再细分到 **16 个**，见 §0.2.2。
**这 7 个都还没推上远端。** 以 `git log --oneline -3` 和 `git status --short` 为准。）
...
- **最新一次提交是 `1742b9f 修作业页回填与提交后判定`**（远端 `main` 已跟上，9 个文件逐个比对字节一致）。
  ...
  与「作业页提交后认出已完成」（9 项）。
- **测试基线：自检 15 项 · 集成 77 项 · 真实 Edge e2e 280 项（26 个场景）· 提示词基准通过**。
...
- ⚠️ **`0f345a2` 修的是上面 `8ce8dae` 没修透的那一半**，值得单列一条：
  ...
  **从来没读过这个字段** —— 拿回 `{success:false, permanentError:true}` 之后
  ...
  测试基线：自检 15 项 · 集成 **77** 项 · 真实 Edge e2e **280 项**（26 个场景）。
...
| 你动了什么 | 必须跑什么 | 判据 |
| --- | --- | --- |
| 任何东西 | `npm test` | **全绿**（自检 15 项 + 提示词基准 + 集成 77 项） |
...
| 当前版本 | **1.2.0**（`page.js` 拆分成 16 个按域片段，见 §0.2.2） |
...
| 测试基线 | 自检 15 项 · 集成 77 项 · 真实 Edge 端到端 **全绿**（项数见 `.workbuddy/e2e-counts.json`，由 `npm run check` 核） · 提示词基准 1 份报告 |
...
| **继续细分大片段（可选）** | `page.js` 已从 9899 行拆成 `src/page/` 下 **16 个按域片段**（§0.2.2），最大的片段 1365 行。剩下 `75-quiz-dom.js`（1365 行）、`40-media.js`（1095 行）、`70-quiz-flow.js`（1042 行）还能再分 | **收益已明显变小，建议就停在这里。** 真要分就**一次只搬一个子域、搬完立刻跑 `npm run e2e`**，并照 `tools/README.md` 里记的四个坑走（尤其"新旧片段重名时清理必须在生成之前"） |
