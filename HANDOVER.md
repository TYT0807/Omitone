...
> `page.js` 已**拆分成 16 个按域片段**（见 §0.2.2，即 1.2.0），
> 之后又做了一轮**按域的代码审计**（即 1.2.1：修两处卡死主循环的 `await` + 一处并发隐患）。

#### ✅ 最新快照（2026-09-23 傍晚）—— 1.2.8 已发布并核验（合入原 1.2.8~1.2.11 全部修复）

**当前版本 1.2.8**：合并了原 1.2.8~1.2.11 同一轮修复 —— 任务点**匹配得上、却跑不起来**（反向反查）的那一半已补齐，并配齐日志逐帧说明 + 弹窗「复制日志」。

**根因**：1.2.8 只改了"正向找任务点"（`_findJobIdInAncestorFrames`），
但任务开跑时还有**反向**一步 `_runChaoxingJob → _resolveJobFrame`，
它读 jobid 的方式**还是老写法**（只在本帧的 `data` / 属性里找）。
于是正向匹配成功、反向反查失败 → `job.doc` 为 null → 任务拿不到页面 → 跑不起来。
**同一个概念在代码里有四处各写了一遍，1.2.8 只改了其中一处。**

**怎么发现的**：给 1.2.8 的测试**补了后半段断言** —— 把匹配到的 job 交给
`_resolveJobFrame`，看能不能反查回帧。**先看到它红**：
`{"jobid":"work-B","before":false,"hasFrame":false,"hasDoc":false}` ——
jobid 是对的、job 确实没帧，但反查之后仍然没帧。327 项里**只有这 1 项**失败。

> ⚠️ **教训（已写进 CHANGELOG）：验证一条修复要验到"链路真的走通了"，
> 而不是"第一步对了"。** 1.2.8 的断言停在"匹配到任务点"，
> 可用户要的是"任务被做掉" —— 中间还隔着"反查回帧 → 拿到 doc → 跑起来"。
> **断言停在半路，等于给修复发了一张假通行证。**
...
**为什么连着 1.2.9 又发一版**：1.2.9 刚把"任务点全被拒"的逐帧原因写进日志，
随即发现一个尴尬的事 —— **日志能看，却送不出来**（200 行只能截长图，
或在弹窗里手工选中，弹窗一失焦还可能被关掉）。
而"整章被跳过""某个任务点不做"这类问题**只能靠日志定位**。
**1.2.9 补了诊断，1.2.10 补的是"把诊断交出来"这最后一环。**
...
原先现场只有一句 `searchedCount: 0`，知道"没匹配上"但**不知道卡在哪一条**，
只能靠人猜（用户报过的「两个大题目只做第一个」当初就是这么反推的）。
七条 `continue` 里有五条**完全静默**，现在全带上原因与帧地址（只记前 8 条）。

> 这条也顺带对齐了 `AGENTS.md` §2 第 7 条「跳过用户的任务点必须留日志」。

**为什么值得现在发**：用户还没做真实平台验证（1.2.3 / 1.2.8 的修复都没实测过）。
**把诊断先发出去**，等他跑的时候，万一还是失败，日志能一次说清卡在哪一步 ——
而不是再来一轮"从 `searchedCount: 0` 反推"。
...
| 测试 | 自检 **18** 项 · 集成 **106** 项 · e2e **31 场景 / 327 项**（落盘 `通过=true`） |
| 新增覆盖 | 场景「任务点全被拒时日志要说明原因」（5 条断言） |
| 反向验证 | 断言直接检查日志内容（逐帧原因 + 帧地址），**先确认返回 null 这个前置条件**，不是只看"有没有打印" |

#### 上一版（1.2.8）的快照 —— 保留作对照

**1.2.8**：修「**jobid 挂在容器 div 上就找不到任务点**」——
症状与 1.2.3 那个**一模一样**（整章被静默跳过），而且**就在同一个函数里**。

**根因是一个不可达的条件**：`_findJobIdInAncestorFrames` 里那段"jobid 在任务点外层容器上"
写的是 `wrap.querySelector('[jobid]')` + `holder === wrap` ——
而 **`querySelector` 只找后代、永远返回不了 `wrap` 自己**，
所以「jobid 写在容器 div 上」这条路径**完全没生效**。

**怎么发现的**：这个错读代码很难确信，所以先按真实结构搭了一个**变体 mock**
（jobid 只在容器 div 上、iframe 上一个属性都不写）加断言 —— **先看它红**：
...
316 项里**只有这 2 项**变红。修法是先看容器自己的 `jobid`/`_jobid`/`data.jobid`，
再退回原来的"后代承载元素"路径；**"必须属于本帧"那条守卫原样保留**
（绑错任务点比匹配不到严重得多），现有那条兄弟帧断言修复后**仍然绿** ✓。

| | |
| --- | --- |
| 本地 / 远端 | **完全一致**（`4785a43`）；tag `v1.2.8` 指向同一个 sha |
| 工作区 | 干净 |
| 线上 Release | **v1.2.8** · `omitone.zip` 370.3 KB（sha256 `67d2ee36f5c9fb71…`）· `Omitone-manual.pdf` 1360.6 KB（sha256 `bfe4528e22e705a3…`），均 `state=uploaded` |
| 下载直链 | 版本直链与永久直链**各下载一次**，两个附件都与本地构建**逐字节一致** |
| 在线说明书 | `tyt0807.github.io/Omitone/docs/manual.html` 已显示 **1.2.8** |
| 测试 | 自检 **18** 项 · 集成 **106** 项 · e2e **31 场景 / 327 项**（落盘 `通过=true`） |
| 新增覆盖 | 场景「jobid 挂在容器 div 上也要能找到」（4 条断言）+ 变体 mock |
| 反向验证 | 先写测试**看到它红**（2 条），再修，再看到全绿（317/317）—— 这条修复的验证顺序与别处相反，**但更可信** |

> ⚠️ **发这一版时被自己的工具误报了一次**（已修）：`verify` 原先读 release 响应里
> **内嵌的 `assets` 字段**，而那个字段实测可能是 `[]` —— 于是把一个**完好的 Release**
> 判成「附件缺失、下载入口是死链」并退出码 1。改成走 `/releases/{id}/assets` 子端点后正常。
> **教训：验证工具报错时，先确认"被验的东西真的坏了"，再信它。**（细节见 CHANGELOG 的 1.2.8 段）

> ⚠️ **1.2.3 与 1.2.8 的修复至今都没在真实平台验证过。**
> e2e 用的是照真实结构搭的 mock；现在覆盖了**两种** jobid 挂法
> （iframe 上 / 容器 div 上），但真实页面用哪种、有没有第三种，
> **只有用户在真实课程里跑一次那张卡片才能确认**。

#### 上一版（1.2.7）的快照 —— 保留作对照

**1.2.7**：接着审 `libs/`，修了两处**静默的错**（都不报错、不留日志）：

| 位置 | 问题 | 用户看得见的症状 |
| --- | --- | --- |
| `normalizeApiKey`（`libs/api-url.js`） | 剥引号与剥 `Bearer` 的**顺序反了** | Key 是对的却一直 **401**（`"Bearer sk-xxx"` → 发出去是 `Bearer Bearer sk-xxx`） |
| `normalizeItem`（`libs/prompt.js`） | 直接 `Number(rawIndex)`，而 **`Number(null)` 是 0** | 模型回 `"i": null` 时答案被错配到第 0 题 → 白跑一轮补问，**多花 token** |

两处都补了集成测试（[11] [12]，集成总数 77 → **106**），并**各自做了反向验证**：
注入旧实现 → 第一处红 10 条、第二处红 3 条，其余保持绿。

> ⚠️ 反向验证时**又栽在同一个坑上**：第一次用 `node -e` 内联多行字符串写注入脚本，
> shell 与 JS 的引号互相截断 → 脚本语法报错 → **什么都没注入** → 测试照样全绿。
> 差点据此得出"断言抓不住"的错误结论。改成写一次性补丁脚本（**带自校验：锚点没匹配就报错退出**）
> 才做对。**判据：注入后先 `node --check` + `grep` 确认落盘，再看断言结果。**

| | |
| --- | --- |
| 本地 / 远端 | **完全一致**（`ddc62f8`）；tag `v1.2.7` 指向同一个 sha |
| 工作区 | 干净 |
| 线上 Release | **v1.2.7** · `omitone.zip` 369.8 KB（sha256 `0cef2d7c0cbc7ccb…`）· `Omitone-manual.pdf` 1360.3 KB（sha256 `1569ebac78f9eee6…`），均 `state=uploaded` |
| 下载直链 | `omitone.zip` 两条直链**各下载一次**、与本地构建**逐字节一致**；`Omitone-manual.pdf` 的**版本直链**逐字节一致 ⚠️（**永久直链没验到** —— 当时 `github.com` 又断了，见下） |
| 在线说明书 | `tyt0807.github.io/Omitone/docs/manual.html` 已显示 **1.2.7** |
| 测试 | 自检 **18** 项 · 集成 **106** 项 · e2e **31 场景 / 327 项**（落盘 `通过=true`） |
| 新增覆盖 | 集成 [11] API Key 清洗（17 条）、[12] index 归一化（12 条）；**打包产物也跑了一遍 e2e** |

> ⚠️ **本轮 `github.com` 反复中断**（`api.github.com` 一直通）：推送重试到**第 11 次**（约 275 秒）
> 才成功；最后验 PDF 永久直链时它又断了，连**几分钟前刚验过的版本直链**也一起失败 ——
> 所以那是**网络问题，不是发布问题**（`verify` 走 API，两个附件都是 `state=uploaded`）。
> 判断顺序见 `AGENTS.md` §7.3：**先重试，别急着改用 REST API**。

#### 上一版（1.2.6）的快照 —— 保留作对照

**1.2.6 把 1.2.4 / 1.2.5 / 1.2.6 三个版本一次带出来** ——
1.2.4 与 1.2.5 当时做完没来得及发布（令牌被 revoke，推不出去），
所以**从 v1.2.3 升上来会同时生效三块改动**：

| 版本 | 内容 |
| --- | --- |
| 1.2.4 | 讨论新开的窗口里插件不自动跑（把"已开启"标记落在 `run()` 里） |
| 1.2.5 | 音频也能拖到结尾（与视频一视同仁） |
| 1.2.6 | 修诊断里写错的选择器大小写 + 三处守卫加固（见下） |

1.2.6 做的事（细节见 `CHANGELOG.md` 的 1.2.6 段）：

- 修 `_describeQuizResultPage` 里容器选择器写错的大小写（`.Cy_TITle` → `.Cy_TItle`）。
  这处修复**当时只改在片段里、没跑 `npm run concat`**，所以一直没上线 ——
  而且**改动前后字节数完全一样**（只差一个字母的大小写），靠比大小发现不了
- 把**手抄在三处**的容器选择器合成一个真源 `_quizContainerSelector`
- 自检新增**第 17 项**（选择器类名大小写一致）与**第 15 项补上状态字段清单的核验**
  （顺带查出 1.2.3 加了 `_quizRunPaperKey` 却没同步头部，头部数字静静地从 119 停在 118）
- 补上 `background.js` 里漏掉的一处超时（验证码图片抓取）
- e2e 补一条断言锁住作业页诊断的容器计数
- **新增 `OMITONE_EXT_DIR`**：e2e 可以直接验**打包产物**（此前只验仓库根目录）

| | |
| --- | --- |
| 本地 / 远端 | **完全一致**（`2be8856`）；tag `v1.2.6` 指向同一个 sha |
| 工作区 | 干净 |
| 线上 Release | **v1.2.6** · `omitone.zip` 369.0 KB（sha256 `473e22cd91267817…`）· `Omitone-manual.pdf` 1361.2 KB（sha256 `e3f7d12fa6640383…`），均 `state=uploaded` |
| 下载直链 | 版本直链与永久直链**各下载一次**，两个附件都与本地构建**逐字节一致** |
| 在线说明书 | `tyt0807.github.io/Omitone/docs/manual.html` 已显示 **1.2.6** |
| 测试 | 自检 **18** 项 · 集成 106 项 · e2e **31 场景 / 327 项**（落盘 `通过=true`） |
| 新增覆盖 | ① 作业页诊断容器计数的 e2e 断言；② **打包产物也跑了一遍 e2e**（31 场景 / 327 项全绿） |
| 反向验证 | 本轮四次注入，全部先 grep 确认落盘：① 选择器大小写改回 → 第 17 项报出两个位置；② 注入并集写法 `.Hedge_Probe, .Hedge_probe` → **不报**（证明收窄有效）；③ 诊断选择器改回 → e2e **327 项里恰好 1 条**变红；④ 头部删字段名 + 数字改错 → 两条同时报出 |

> ⚠️ **1.2.3 的修复至今没在真实平台验证过**：e2e 用的是照真实结构搭的 mock 页。
> 用户需要在真实课程里再跑一次那张卡片（两个大题目：第六单元检测 / 章节测试），确认第二份被做掉。

> 📌 **本轮踩到的网络坑**：`git push` 一度全线失败（`Failed to connect to github.com:443`），
> 几分钟后重试即成功；而且 `uploads.github.com` 的**根路径**探活会给出假信号。
> 判断顺序已写进 `AGENTS.md` §7.3 —— **先重试，别急着改用 REST API**。

#### 上一版（1.2.3）的快照 —— 保留作对照

**1.2.3 修的是「一张学习卡片里两份测验任务点，第二份不做」。**

根因有两个，**缺一个都修不好**：

1. **任务点身份（jobid）和题目（`.TiMu`）不在同一层帧里** —— 真实结构三层：外层
   `/ananas/modules/work/index.html` 带 `data`（jobid）但没有 `.TiMu`，最内层
   `/mooc-ans/work/doHomeWorkNew?…` 有整卷题目但 `data` 为空。
   `_searchChaoxingJobOcs` 只认"本帧自己的 `data`"→ **三层全被 `continue`** →
   `searchedCount: 0` → 它认为"本页没有任务点"→ `nextUnit()` 把整章推过去。
2. **"已答过"是页面级的** —— `_quizAnswered` 只在 `_resetRuntimeState()` 里清，
   而调用点只有 `run()` / 换章节 / `nextUnit()` / 换学习卡片；
   **同一张卡片里换一份卷子一次都不经过**，于是 `_handleQuiz` 第一行就 return。

修法：`_findJobIdInAncestorFrames`（沿祖先帧链找 jobid，取不到仍返回空串不瞎猜）
+ `_quizRunPaperKey` / `_syncQuizPaperRunState`（换卷子才重置，三道保险）。
细节见 `CHANGELOG.md` 的 1.2.3 段。

| | |
| --- | --- |
| 本地 / 远端 | **完全一致**（`45a14a1`）；tag `v1.2.3` 指向同一个 sha |
| 工作区 | 干净 |
| 线上 Release | **v1.2.3** · `omitone.zip` 366.8 KB（sha256 `460e7784812cd571…`）· `Omitone-manual.pdf` 1361.0 KB（sha256 `4dbc703bfabb1445…`），均 `state=uploaded` |
| 下载直链 | 永久直链与版本直链**各下载一次**，都与本地构建**逐字节一致** |
| 在线说明书 | `tyt0807.github.io/Omitone/docs/manual.html` 已显示 **1.2.3**（页面里 5 处 1.2.3、0 处 1.2.2） |
| 测试 | 自检 **18** 项 · 集成 106 项 · e2e **31 场景 / 327 项** |
| 新增覆盖 | 场景「一张卡片两份测验：第二份也要被匹配到并答」（18 条断言，mock 页按实测的三层结构搭） |
| 反向验证 | 三次注入：① 去掉跨层查找 → 匹配为 null；② 让"换卷子"永不重置 → 3 条红；③ 去掉"必须属于本帧"收窄 → 报出拿走了兄弟的 jobid。三次都先 grep 确认注入落盘再看结果 |

> ⚠️ **这一版同样没在真实平台验证过**：e2e 用的是照真实结构搭的 mock 页。
> 用户需要在真实课程里再跑一次那张卡片（两个测验任务点），确认第二份被做掉。

> 📌 **本文件的测试数字守卫扫全篇**（`tools/check.js` 的 `checkTestCounts`），
> 所以下面**每一段（含历史快照）**的 e2e 数字都保持为**当前基线**。
> 想看某一版当时的数字，请查 `CHANGELOG.md` 对应段落。

#### 上一版（1.2.2）的快照 —— 保留作对照，内容已被上面覆盖

**1.2.2**：在 1.2.1 的审计之后又查出**一处更易触发的并发隐患** ——
看门狗的判据是「tick 活了多久」而不是「多久没有进展」，于是
**正常干活的长任务会被误判成卡死并强制放锁**（`_waitSlideAudioDone` 单页上限 600 秒，
远超看门狗的 150 秒），放锁后新旧两个 tick 会同时操作同一个任务点。

修法：新增进度心跳 `_tickProgress(note)`（长循环主动刷新），判据改为
`now - (_tickProgressAt || _tickStartedAt) > 150000`。**`_tickEpoch` 与 finally 一字未动。**
细节见 `CHANGELOG.md` 的 1.2.2 段。

| | |
| --- | --- |
| 本地 / 远端 | 见下（本次推送后核对） |
| 测试 | 自检 **18** 项 · 集成 106 项 · e2e **31 场景 / 327 项** |
| 新增覆盖 | 场景「看门狗：按"无进展"而非"总时长"判卡死」（6 条断言，补上了这块此前的零覆盖） |
| 反向验证 | 两次注入：① 看门狗失效 → 2 条变红（并暴露出第 ① 条断言覆盖不全，已补 ③④）；② 判据改回总时长 → 3 条变红，报出 `running:false, epoch:101` 即被误杀的直接证据 |

> ⚠️ **这一版同样没在真实平台验证过**：看门狗属于"跑得久才触发"的路径，
> 用户需在真实课程里跑一次带 PPT 音频的任务点才能确认。日志里现在有
> `tick progress` 条目（带 `note` 与 `tickMs`），可据此看到长任务实际耗时。

#### 上一版（1.2.1）的快照 —— 保留作对照，内容已被上面覆盖

**1.2.1**：1.2.0 之后按域做了一轮代码审计，修了三处问题
（两处会让主循环卡住的 `await`、一处看门狗强制放锁引入的并发隐患），
并新增自检第 16 项（`await` 必须可超时）。细节见 `CHANGELOG.md` 的 1.2.1 段。

**已推送并发布**：`git push` 一次推上去 5 个提交（`f3129d2..ecdee29`，历史完整）。
本地 HEAD、远端 `main`、tag `v1.2.1` **三者同为 `ecdee29`**；
`verify v1.2.1` 退出码 0。

| | |
| --- | --- |
| 本地 / 远端 | **完全一致**（`ecdee29`）；tag `v1.2.1` 指向同一个 sha |
| 工作区 | 干净 |
...
#### 上一版（1.2.0）的快照 —— 保留作对照，内容已过期

**1.2.0 的改动是 `page.js` 的拆分**（行为不变，见 §0.2.2）+ 自检第 15 项（片段头部守卫）。
发版前审查（`npm run audit:publish`）通过、`npm test` 与 `npm run e2e`（31 场景 / 327 项）全绿。
推送用的是"令牌放进 URL"那条路（AGENTS §7.3），不是 REST API —— API 会把多个提交压成一个。
...
| 测试 | 自检 **18** 项（那一版当时是 15 项，还没加第 16 项）· 集成 106 项 · e2e **31 场景 / 327 项** |
...
- **最新一次提交是 `1742b9f 修作业页回填与提交后判定`**（远端 `main` 已跟上，9 个文件逐个比对字节一致）。
  ...
  端到端现在是 **31 个场景 / 327 项**，新增「作业页选项写入隐藏答案域」（8 项）
  与「作业页提交后认出已完成」（9 项）。
- **测试基线：自检 18 项 · 集成 106 项 · 真实 Edge e2e 327 项（31 个场景）· 提示词基准通过**。
...
- ⚠️ **`0f345a2` 修的是上面 `8ce8dae` 没修透的那一半**，值得单列一条：
  ...
  **从来没读过这个字段** —— 拿回 `{success:false, permanentError:true}` 之后
  ...
  测试基线：自检 18 项 · 集成 **106** 项 · 真实 Edge e2e **327 项**（31 个场景）。
...
### 0.2.3 `page.js` 按域审计（1.2.1）

拆分的**直接收益**在这里兑现了：拆之前"有没有会挂死的 await"只能靠通读近万行，
拆之后可以**按域逐个审**。1.2.1 就是第一轮的结果 —— 查出三处问题：

| 问题 | 位置 | 性质 |
| --- | --- | --- |
| `await fetch` / 读响应体**无超时** | `_maybeRepairMediaSource`（`40-media.js`） | 服务器挂起 → 最长 150 秒插件不干活 |
| 同上 | `_resolveDiscussionUrlFromModule`（`65-discussion.js`） | 同上（外面有 `try/catch` 也拦不住**挂起**） |
| 看门狗强制放锁后**旧 tick 复位新 tick 的锁** | `_runTick` 的 `finally`（`64-tasks-loop.js`） | 可能两个 tick 并行（重复提交、重复跳章） |

顺带补上两个**从未声明**的状态字段（`_tickRunning` / `_tickStartedAt`）。

**沉淀下来的东西**：

- **自检第 16 项**：`await` 必须可超时（只盯 `fetch` / 读响应体 / 媒体 `play`，
  要求同一行有 `_withTimeout(`）。反向验证过。
- **`tools/audit-safedoc.js`**：按需审计"裸读跨域 `.document` 有没有被 `try` 包住"。
  最近一次结论：**16 处全部有 `try`，0 处危险**。
- **`_tickEpoch` 代号**：改 `_runTick` 的 `finally` 时**别删那个代号判断**（README §5.1）。

> ⚠️ **这一轮最该记住的教训：审计工具本身会骗你。**
> 第一版跨域分析器的词法器漏了**正则字面量** —— `60-tasks-detect.js` 里一个含引号的正则
> 让它以为进了字符串，于是那份文件后面的行全被剥成空，**4 处裸读一处都没报出来**，
> 而汇总行照样打印"没有保护 0 处"。
> 所以：**看"检查通过了"之前，先确认"检查真的跑了"** —— 这也是它没进 `npm test` 的原因
> （启发式词法器不适合当每次都要跑的硬门禁），现在脚本自带"原文命中数 vs 剥注释后命中数"的自校验。
...
| 你动了什么 | 必须跑什么 | 判据 |
| --- | --- | --- |
| 任何东西 | `npm test` | **全绿**（自检 18 项 + 提示词基准 + 集成 106 项） |
...
| 测试基线 | 自检 18 项 · 集成 106 项 · 真实 Edge 端到端 **全绿**（项数见 `.workbuddy/e2e-counts.json`，由 `npm run check` 核） · 提示词基准 1 份报告 |
...
| 项 | 说明 | 建议 |
| --- | --- | --- |
| **最近几版的修复都没在真实平台验证过** | 按时间排：「作业页回填隐藏域」「提交后认判分结果页」（1.2.3）→ jobid 挂在容器 div 上（1.2.8）→ 任务点反查回帧（1.2.11）。**全部只过了 mock e2e**（327 项全绿 + 反向验证），真实学习通页面**用户尚未回归实测** | 等用户跑一次真实页面。若仍有异常，**先看日志再改代码**：① 整章被跳过 → 看 1.2.9 加的**逐帧被拒原因**（`searchedCount: 0` 下面那几行，带帧地址）；② 匹配上了却跑不动 → 看 `job.doc` 是否为 null；③ 作业/考试页异常 → 按 README §7 调试手册的两行（`#answer{qid}` 空串 / 停在 `waiting quiz submit result`）定位 |
