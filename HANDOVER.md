...
| 测试 | 自检 **16** 项 · 集成 77 项 · e2e **29 场景 / 311 项** |
...
发版前审查（`npm run audit:publish`）通过、`npm test` 与 `npm run e2e`（29 场景 / 311 项）全绿。
...
| 测试 | 自检 **15** 项（当时还没加第 16 项）· 集成 77 项 · e2e **29 场景 / 311 项** |
...
| 项 | 说明 | 建议 |
| --- | --- | --- |
| **本轮修复还没在真实平台验证过** | 「作业页回填隐藏域」「提交后认判分结果页」两个修复只过了 mock e2e（311 项全绿 + 反向验证），真实学习通作业/考试页**用户尚未回归实测** | 等用户跑一次真实作业页。若仍有异常，先按 README §7 调试手册新增的两行（`#answer{qid}` 空串 / 停在 `waiting quiz submit result`）定位，别直接改代码 |
