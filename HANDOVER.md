...
| 测试 | 自检 **17** 项 · 集成 **106** 项 · e2e **29 场景 / 312 项**（落盘 `通过=true`） |
...
| 测试 | 自检 **16** 项 · 集成 106 项 · e2e **29 场景 / 312 项** |
...
| 测试 | 自检 **15** 项（当时还没加第 16 项）· 集成 106 项 · e2e **29 场景 / 312 项** |
...
- **最新一次提交是 `1742b9f 修作业页回填与提交后判定`**（远端 `main` 已跟上，9 个文件逐个比对字节一致）。
  ...
  与「作业页提交后认出已完成」（9 项）。
- **测试基线：自检 17 项 · 集成 106 项 · 真实 Edge e2e 312 项（29 个场景）· 提示词基准通过**。
...
- ⚠️ **`0f345a2` 修的是上面 `8ce8dae` 没修透的那一半**，值得单列一条：
  ...
  **从来没读过这个字段** —— 拿回 `{success:false, permanentError:true}` 之后
  ...
  测试基线：自检 17 项 · 集成 **106** 项 · 真实 Edge e2e **312 项**（29 个场景）。
...
| 你动了什么 | 必须跑什么 | 判据 |
| --- | --- | --- |
| 任何东西 | `npm test` | **全绿**（自检 17 项 + 提示词基准 + 集成 106 项） |
...
| 测试基线 | 自检 17 项 · 集成 106 项 · 真实 Edge 端到端 **全绿**（项数见 `.workbuddy/e2e-counts.json`，由 `npm run check` 核） · 提示词基准 1 份报告 |
