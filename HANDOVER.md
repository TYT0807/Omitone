...
- ⚠️ **`0f345a2` 修的是上面 `8ce8dae` 没修透的那一半**，值得单列一条：
  ...
  **从来没读过这个字段** —— 拿回 `{success:false, permanentError:true}` 之后
  ...
  而 500 对照仍绿；恢复后 245/245 全绿。
  测试基线：自检 13 项 · 集成 **77** 项 · 真实 Edge e2e **245 项**（24 个场景）。
...
| 测试基线 | 自检 13 项 · 集成 67 项 · 真实 Edge 端到端 **全绿**（项数见 `.workbuddy/e2e-counts.json`，由 `npm run check` 核） · 提示词基准 1 份报告 |
