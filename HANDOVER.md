...
#### ✅ 最新快照（2026-09-23 傍晚）—— 1.2.10 已发布并核验

**当前版本 1.2.10**：弹窗加**「复制日志」** —— 把日志整段复制成纯文本，粘贴就能发出来。
...
| 测试 | 自检 **18** 项 · 集成 **106** 项 · e2e **31 场景 / 325 项**（落盘 `通过=true`） |
...
| 测试 | 自检 **18** 项 · 集成 106 项 · e2e **31 场景 / 325 项**（落盘 `通过=true`） |
| 新增覆盖 | ① 作业页诊断容器计数的 e2e 断言；② **打包产物也跑了一遍 e2e**（31 场景 / 325 项全绿） |
| 反向验证 | 本轮四次注入，全部先 grep 确认落盘：① 选择器大小写改回 → 第 17 项报出两个位置；② 注入并集写法 `.Hedge_Probe, .Hedge_probe` → **不报**（证明收窄有效）；③ 诊断选择器改回 → e2e **325 项里恰好 1 条**变红；④ 头部删字段名 + 数字改错 → 两条同时报出 |
...
| 测试 | 自检 **18** 项 · 集成 106 项 · e2e **31 场景 / 325 项** |
...
发版前审查（`npm run audit:publish`）通过、`npm test` 与 `npm run e2e`（31 场景 / 325 项）全绿。
...
| 测试 | 自检 **18** 项（那一版当时是 15 项，还没加第 16 项）· 集成 106 项 · e2e **31 场景 / 325 项** |
...
- **最新一次提交是 `1742b9f 修作业页回填与提交后判定`**（远端 `main` 已跟上，9 个文件逐个比对字节一致）。
  ...
  端到端现在是 **31 个场景 / 325 项**，新增「作业页选项写入隐藏答案域」（8 项）
  与「作业页提交后认出已完成」（9 项）。
- **测试基线：自检 18 项 · 集成 106 项 · 真实 Edge e2e 325 项（31 个场景）· 提示词基准通过**。
...
- ⚠️ **`0f345a2` 修的是上面 `8ce8dae` 没修透的那一半**，值得单列一条：
  ...
  **从来没读过这个字段** —— 拿回 `{success:false, permanentError:true}` 之后
  ...
  测试基线：自检 18 项 · 集成 **106** 项 · 真实 Edge e2e **325 项**（31 个场景）。
...
| 项 | 说明 | 建议 |
| --- | --- | --- |
| **本轮修复还没在真实平台验证过** | 「作业页回填隐藏域」「提交后认判分结果页」两个修复只过了 mock e2e（325 项全绿 + 反向验证），真实学习通作业/考试页**用户尚未回归实测** | 等用户跑一次真实作业页。若仍有异常，先按 README §7 调试手册新增的两行（`#answer{qid}` 空串 / 停在 `waiting quiz submit result`）定位，别直接改代码 |
