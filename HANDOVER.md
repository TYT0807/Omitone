...
（快照写于 2026-09-21 傍晚；之后又加了「page.js 拆分提示词」等文档提交。
...
### 0.2.2 待办：`page.js` 拆分（方案已就绪，等你执行）
...
>
> 那份文档是**自包含的** —— 可以直接整份复制给另一个 AI，它不需要看这段对话。
> 里面写全了：现状勘明（含各域的行数/跨度统计）、两阶段方案、九条硬性约束、
> 验收清单、以及本仓库实测过的"假成功"陷阱。

**核心设计（先看这个，别急着切）**：分两阶段，第一阶段**零行为风险** ——

1. **阶段一**：按**物理行区间**把 `page.js` 切成 `src/page/*.js`（**只切不改，一行都不动**），
   再加一个拼接脚本产出根目录的 `page.js`。
   **验收标准：拼接产物与拆分前逐字节相同**（`cmp` 无输出）。
   这一步之后 `manifest.json` / `content.js` / `web_accessible_resources` /
   `build.js` 的 `INCLUDE` **全都不用改** —— 因为它们看的还是根目录那个 `page.js`。
2. **阶段二**：按域重组（一次只搬一个域，**搬完立刻跑 `npm run e2e`**）。
...
**做完请自问**：`README.md` 的 §8 定位表能不能让人从"我要改倍速"直接跳到 `media.js`？
如果还要 grep 一遍，那这次拆分只是把一个大文件切成了几个没有导航的碎片，等于白做。
...
| 不要做 | 原因 |
| --- | --- |
| **不要重构 `page.js`** | 9500+ 行 / 320 多个方法的巨石，分割收益大但风险更高。真要做，先补测试再动（§9） |
...
2. **别通读 `page.js`** —— 9500+ 行里绝大多数与你的任务无关。用本文 §3 的索引 +
   搜索定位，读文件时只读目标函数附近。
