...
实测踩过：`30-log.js` 的容器计数写成 `.Cy_TITle`，而真源 `_questionSelectors` 里是 `.Cy_TItle`。
后果是 `xxtAI.diagnose()` 的 `containers` **永远报 0** —— 而诊断工具正是排查
...
逐个访问本地 mock 页面，**逐功能交叉检验实际行为**。当前 29 个场景 / 311 项断言。
