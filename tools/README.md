...
13. **测试数字守卫** —— 文档里写的场景数 / e2e 断言总数 / 集成条数 / 自检项数
    必须等于实测值。三个总数静态都数不出来（e2e 与集成含动态断言，
    自检项数则是"执行段里调了几个 `checkXxx()`"），来源各不相同：

    | 数字 | 实测值从哪来 |
    | --- | --- |
    | 场景数 | 直接数 `browser-e2e.js` 里的 `SCENARIOS.push(` |
    | e2e 断言总数 | e2e 落盘 `.workbuddy/e2e-counts.json` |
    | 集成条数 | itest 落盘 `.workbuddy/itest-counts.json` |
    | 自检项数 | 数 `check.js` **执行段**里的 `checkXxx()` 调用（定义段不算） |

    **没跑过 e2e / itest 就跳过不判错** —— 不能因为"文件不存在"就判失败。

    > 为什么后三个也要守：它们同样散在十几处文档里。
    > 1.2.7 把集成条数 77 → 106 是**手工改了 14 处**；加第 18 项自检又要手工同步 11 处 ——
    > 全靠人眼，漏一处没有任何提示。
    >
    > ⚠️ **"集成"那条判据是"很短一段内"而不是"关键词之后的第一个数字"。**
    > 用后者的话，正文里凡是提到"集成"的行都会把无关数字当成集成条数 ——
    > 实测**被自己的说明文字误报过 3 次**（比如「把集成条数 77 → 106 改了一遍…
    > 加第 18 项自检」）。现在写成 `集成[^。\n\d]{0,6}(\d+)\**\s*项`：
    > 6 个字符够覆盖「集成测试（106 项）」，够不着下一句；间隔里也不许出现数字
    > （否则 `集成 106 项` 会被拆成"间隔吃掉 10、数字只剩 6"）。
    >
    > ⚠️ **自检项数用的是"正向匹配"**（`自检` 紧跟数字 + 项），又与前两条**不同** ——
    > 因为文档里有「`npm test  # 自检 + 提示词基准 + 集成测试（…项）`」这种
    > **没给自检报数**的行，沿用前两条会把集成条数误当成自检项数。
    >
    > ⚠️ **它不区分"声明"和"举例"**：正文里引用一条报错示例（"…写的是 99 项…"）
    > 也会被当成文档声明扫到 —— 写 CHANGELOG 时被自己的守卫拦过一次。
    > 所以**一行里只放一个数字**。
14. **`page.js` 与 `src/page/` 一致** —— 见上文「拼接机制」。拦的是"直接改根目录产物、
    下次拼接全丢"这种**跑起来是对的**的静默失效
15. **片段头部与实际一致** —— 每个片段的头部注释必须与文件里真实的 `app` 成员一一对上：
    - `片段 NN/MM` 序号（NN 按文件名排序，MM 是最后一位）
    - 「本段的方法（N 个）」清单（**多行**排布）
    - 「本段的状态字段（N 个）」清单（**同一行**排布）

    头部长了短了、方法/字段搬走了没改头部，读的人就会照着错的地图找代码 ——
    **有错地图比没地图更糟**（没地图时他会 grep，有错地图时他会信）。
    `window.xxtAI` 上那 8 个入口不算 `app` 成员，判据里已排除。

    > ⚠️ **状态字段清单是后补上的核验。** 原先只核方法清单，于是 1.2.3 往
    > `10-config-state.js` 加了 `_quizRunPaperKey` 却没同步头部，
    > **头部数字静静地从 119 停在 118** —— 没人发现。现在两种清单都核，
    > 且数字与成员名都比（实测注入"删掉一个字段名 + 数字改错"两条同时报出）。

    反向验证过：漏写一个方法、写一个不存在的方法、序号错位、删掉一个状态字段、
    状态字段计数写错 —— 各自都被抓住。
16. **`await` 必须可超时** —— 见下方「为什么单独守这一条」
17. **选择器类名大小写一致** —— **CSS 类名大小写敏感**，`.Cy_TITle` 与 `.Cy_TItle` 是两个不同的类，
    写错的那个 selector **匹配不到任何元素，且不报错、不留日志**。判据：把类名按小写分组，
    同组出现多种拼写即报错。两条必要的收窄见下方专节
18. **桥接消息类型两端一致** —— `page.js` 与 `content.js` 靠 `window.postMessage` 通信、
    **按字符串类型分发**。任何一端把类型名拼错，另一端就**收不到、也不报错**，
    唯一表现是 `bridgeSend` 那条 promise 一直等到 **90 秒超时**，
    日志里写的是「bridge timeout」——**把人指向"网络/桥接坏了"**，而真因是一个拼错的字符串。

    两个方向都比对：① page 用 `bridgeSend('X')` 发出的，content 必须有对应分支；
    ② content 用 `source: 'xxt_bridge'` 发出的，page 必须有对应分支。

    > ⚠️ **收类型时两条路都要收**：`bridgeSend('X')`（要等回执的）**和**
    > 直接 `window.postMessage({ source: 'xxt_app', type: 'X' })`（不等回执的，
    > 例如 `emitRuntimeLog` 的 `runtime_log`、`10-config-state.js` 的 `storage_set`）。
    > 只收前者会漏掉后两种（实测漏了 2 种）。
    >
    > 守卫带**自保护**：某一侧收 0 种时直接判失败（"守卫失效比没有守卫更糟"），
    > 而不是静默通过。反向验证过：把提取正则改到匹配不上 → 报「守卫失效」。

失败时退出码为 1，可直接用于 CI。

## 为什么单独守"选择器类名大小写"

实测踩过：`_describeQuizResultPage`（`30-log.js`）里**手抄的一份**容器选择器写成 `.Cy_TITle`，
而真源 `_questionSelectors` 里是 `.Cy_TItle`。后果是那条「quiz submit wait timeout」日志里的
`containers` 少算、在只用 `.Cy_TITle` 的作业/考试页上直接报 0 ——
而那条日志**唯一的作用**就是告诉人"没认出结果页时缺的是哪一道条件"。
**诊断工具骗人比没有诊断更糟**：看日志的人会以为页面上没有题目容器，往错的方向查。
（这种错法不看代码是发现不了的：不抛异常、不留日志、页面上也看不出。）

> ⚠️ 同一个文件里 `xxtAI.diagnose()` 的 `containers` 走的是
> `_collectQuestionContainers()` → `_questionSelectors` **真源**，**没被这个笔误影响**。
> 真相是**两个诊断各算各的** —— 一个走真源（对），一个手抄了一份（错）。
> 这正是"选择器清单只写一处、别在多个分支里各抄一份"那条规矩存在的理由。

判据是"按小写分组，同组多拼写即报错"，但**必须收窄两处**，否则全是误报（都实测过）：

| 收窄 | 为什么 | 实例 |
| --- | --- | --- |
| 只看普通的 `.Foo` 选择器，**不看 `[class*="Foo"]`** | 第三方 UI 库类名大小写不确定时，本仓库既定写法是两套都列上 | `[class*="player"], [class*="Player"]`（`40-media.js`）、`[class*="speed"], [class*="Speed"]`、`[class*="dialog"], [class*="Dialog"]`（`50-captcha.js`） |
| 同组拼写**出现在同一行**的不算冲突 | 那是"并集写法"，两套都列了，不是笔误 | `.icon_Completed, .icon_completed`（`60-tasks-detect.js`） |

收窄之后，只有"**两个地方各写了一个大小写不同的普通类名**"才会报 —— 而那正是真 bug 的形态。

反向验证过两个方向：① 把 `.Cy_TITle` 改回 `.Cy_TITle` → 精准报出两个位置；
② 注入一行 `.Hedge_Probe, .Hedge_probe` 并集写法 → **不报**（证明收窄有效，守卫不会因噪声被人关掉）。
...
message 监听器驱动完整答题往返。跑完会把实测条数落盘到 `.workbuddy/itest-counts.json`，
供 `npm run check` 核对文档里写的「集成 N 项」—— 机制与 e2e 那份计数文件同款。
...
逐个访问本地 mock 页面，**逐功能交叉检验实际行为**。当前 30 个场景 / 317 项断言。
