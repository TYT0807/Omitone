...
13. **测试数字守卫** —— 文档里写的场景数 / e2e 断言总数 / 集成条数必须等于实测值
    （两个总数都静态数不出来，由测试自己落盘：e2e → `.workbuddy/e2e-counts.json`，
    集成 → `.workbuddy/itest-counts.json`；**没跑过就跳过不判错**，
    不能因为"文件不存在"就判失败）

    > 为什么集成条数也要守：它同样散在十几处文档里（`AGENTS.md` / `README.md` /
    > `tools/README.md` / `HANDOVER.md` / `CHANGELOG.md` 最新段）。
    > 1.2.7 那次 77 → 106 是**手工改了 14 处**，全靠人眼，漏一处没有任何提示。
    >
    > ⚠️ 判据里 `\**` 不能省：文档里数字常常是**加粗**的（`集成 **106** 项`），
    > 写成 `(\d+)\s*项` 会让正则**跳到行里下一个数字**去
    > （实测把 e2e 的 312 当成了集成条数）。
    >
    > ⚠️ 它**不区分"声明"和"举例"**：正文里引用一条报错示例（"…写的是 99 项…"）
